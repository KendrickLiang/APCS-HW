...
public class StatPrinter {
	...
	for (int x = 0; x < max(data); x++){
	    matchCtr = 0;
	    for (int y = 0; y < data.size(); y++){
		if (data.get(y) == x)
		    matchCtr += 1;
	    }
	    _frequency.add(matchCtr);
	}
    }
    ...
    { 
	return (_frequency.get(i) > _frequency.get(i+1) && _frequency.get(i) > _frequency.get(i-1) && i > 0 && i < _frequency.size() - 1);
    }
	...
	for (int s = 1; s < _frequency.size() - 1; s++){
	    if (isLocalMode(s))
		retArr.add(_frequency.get(s));
	}
	return retArr;
    }
    ...
    {
	int propVal = (longestBar / max(_frequency));
	String bar = "";
	for (int t = 0; t < _frequency.size(); t++){
	    bar = "";
	    _frequency.set(t, _frequency.get(t) * propVal);
	    for (int u = 0; u < _frequency.get(t); u++)
		bar += "*";
